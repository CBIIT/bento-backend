...
class VPCResources:
  def createResources(self, ns):
    ...
    # VPC
    bentoVPC = ec2.Vpc(self, "bento-vpc", cidr=self.config[ns]['vpc_cidr_block'])
    ...
    for netID,zoneID, in privateSubnets.items():
      ec2.PrivateSubnet(self, 'private' + netID.replace('/', '_'), cidr_block=netID, availability_zone=zoneID, vpc_id=bentoVPC.vpc_id, )
    ...
    for netID,zoneID, in publicSubnets.items():
      ec2.PublicSubnet(self, 'public' + netID.replace('/', '_'), cidr_block=netID, availability_zone=zoneID, vpc_id=bentoVPC.vpc_id, )