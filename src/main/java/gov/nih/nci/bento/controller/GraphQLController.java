...
public class GraphQLController {
	...
	public ResponseEntity<String> getGraphQLResponse(HttpEntity<String> httpEntity, HttpServletResponse response) {
		...
		if ((operation == Operation.QUERY && config.isAllowGraphQLQuery()) ||
			...
			try {
				...
				try {
					databaseResponse = executeDatabaseRequest(graphQLWrapper.getDatabaseRequest());
				} catch (ApiError apiError) {
					return logAndReturnError(apiError.getStatus(), apiError.getMessage(), apiError.getErrors());
				}
				//Execute service request
				String serviceResponse = executeServiceRequest(graphQLWrapper);
				...
				if (serviceResponse != null && databaseResponse != null) {
					try {
						JSONObject mergedData = mergeResponses(serviceResponse, databaseResponse);
					...
					String error = "No response was generated by the query\n" +
							"database query: " + graphQLWrapper.getDatabaseRequest();
					return logAndReturnError(status, error);
	...
	private String executeDatabaseRequest(String databaseRequest) throws ApiError {
		String databaseResponse = null;
	...
	private String executeServiceRequest(GraphQLWrapper graphQLWrapper){
	...
	private JSONObject mergeResponses(String serviceResponse, String databaseResponse){
		JSONObject mergedData = new JSONObject();
		JSONObject dataService = new JSONObject(serviceResponse);
		...
		JSONObject dataDatabase = new JSONObject(databaseResponse);
	...
	private ResponseEntity logAndReturnError(HttpStatus status, String message, List<String> errors) {
		...
		newMessage += "\n";
		return logAndReturnError(status, newMessage);
	}
