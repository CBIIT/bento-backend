...
public class AWSClient extends AbstractClient {
    ...
    public org.opensearch.client.RestClient getLowLevelElasticClient() {
        ...
        signer.setRegionName(config.getRegion());
        HttpRequestInterceptor interceptor = new AWSRequestSigningApacheInterceptor(config.getServiceName(), signer, credentialsProvider);
        return org.opensearch.client.RestClient.builder(new HttpHost(config.getEsHost().trim(), config.getEsPort(), config.getEsScheme())).setHttpClientConfigCallback(hacb -> hacb.addInterceptorLast(interceptor)).build();
    }
}
