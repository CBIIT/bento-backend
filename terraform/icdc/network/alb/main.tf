...
module "dev_alb" {
  ...
  alb_security_groups = ["${data.terraform_remote_state.network.public_security_id}"]
  alb_subnets         = ["${data.terraform_remote_state.network.public_subnet_id}","${data.terraform_remote_state.network.public_subnet_c_id}"]
